...
class _AdminAnalyticsScreenState extends State<AdminAnalyticsScreen> {
  ...
  final List<String> _departments = [
    'Semua Departemen', 'IT', 'HR', 'Marketing', 'Finance', 'Operations'
  ];
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _recommendations = _generateRecommendations(results[0]['data']['stats']);
    ...
    return [
      {
        ...
        'description': '${userStats['active']} active users currently in system',
        'type': 'users',
        'is_positive': userStats['active'] > 0,
      },
      {
        'title': 'Dataset Processing Rate',
        'value': '${datasetStats['success_rate']?.toStringAsFixed(1) ?? '0.0'}%',
        'description': '${datasetStats['processed']} of ${datasetStats['total']} datasets processed successfully',
        'type': 'processing',
      ...
      },
      {
        ...
        'description': 'System administrators managing ${userStats['employees']} employees',
        'type': 'admin',
  ...
  List<Map<String, dynamic>> _generateRecommendations(Map<String, dynamic> stats) {
    ...
    if (userStats['admins'] < (userStats['total'] / 10)) {
      recommendations.add({
        ...
        'description': 'Current admin-to-user ratio is low. Consider promoting qualified users to admin role.',
        'timeline': 'This week',
    ...
    if (datasetStats['failed'] > 0) {
      recommendations.add({
        ...
        'description': '${datasetStats['failed']} datasets failed processing. Review data quality guidelines.',
        'timeline': 'Next week',
    ...
    if (userStats['inactive'] > 0) {
      recommendations.add({
        ...
        'description': '${userStats['inactive']} inactive users found. Consider account cleanup.',
        'timeline': 'This month',
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _getPriorityColor(rec['priority'] as String).withOpacity(0.1),
        ...
        ...factors.map((rowFactor) {
          return TableRow(
            children: [
              Padding(
                padding: const EdgeInsets.all(4),
                child: Text(rowFactor, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 10)),
              ),
  ...
  double _getCorrelation(String factor1, String factor2) {
    ...
    if ((factor1 == 'Workload' && factor2 == 'Work-Life Balance') ||
        (factor1 == 'Work-Life Balance' && factor2 == 'Workload')) return 0.8;
